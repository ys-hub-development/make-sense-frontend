...
import { ExportDBBBox, ExportDBSegmentation } from '../../export-db/types';
...
export class COCOExporter {
  ...
  public static getCOCOSegmentation(vertices: IPoint[]): ExportDBSegmentation {
    const points: number[][] = vertices.map((point: IPoint) => [point.x, point.y]);
    return [flatten(points)];
  }

  public static getCOCOBbox(vertices: IPoint[]): ExportDBBBox {
