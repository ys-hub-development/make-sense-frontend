...
interface IProps {
  ...
  updateHighlightedLabelId: (highlightedLabelId: string) => void;
  updateActiveLabelId: (highlightedLabelId: string) => void;
  updateActivePopupType: (activePopupType: PopupWindowType) => void;
...
class LabelInputField extends React.Component<IProps, IState> {
  ...
  private getDropdownStyle = (): React.CSSProperties => {
    ...
    const height: number =
      Math.min(this.props.options.length, this.dropdownOptionCount) * this.dropdownOptionHeight + 30;
    const style = {
      width: clientRect.width,
      height,
  ...
  public render() {
    const { size, id, value, onDelete, onEdit } = this.props;
    return (
      <div
