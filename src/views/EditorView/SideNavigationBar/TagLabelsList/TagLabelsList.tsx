...
interface IProps {
  size: ISize;
  imageData: ImageData;
  updateImgDataById: (id: string, newImageData: ImageData) => any;
  labelNames: LabelName[];
  changeActivePopupType: (activePopupType: PopupWindowType) => any;
}
...
const TagLabelsList: React.FC<IProps> = (
  ...
  const onTagClick = (labelId: string) => {
    if (imageData.labelNameIds.includes(labelId)) {
      updateImgDataById(imageData.id, {
        ...imageData,
  ...
  const addNewOnClick = () => {
    changeActivePopupType(PopupWindowType.UPDATE_LABEL);
  };
