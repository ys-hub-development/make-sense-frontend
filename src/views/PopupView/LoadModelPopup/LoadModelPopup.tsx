...
const LoadModelPopup = ({ activePopupType }: IProps) => {
  ...
  useEffect(() => {
    if (PopupWindowType.LOAD_AI_MODEL_DB === activePopupType) {
      updateSelectedModelToLoad([
        {
          model: AIModel.OBJECT_DETECTION,
          name: 'COCO SSD - object detection using rectangles',
          flag: false
  ...
  const getOptions = () => {
    return selectedModelToLoad.map((entry, idx) => {
      return <div
